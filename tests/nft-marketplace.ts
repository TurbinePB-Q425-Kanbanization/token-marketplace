...
describe("nft marketplace — token2022", () => {
    ...
    const sellerAta = getAssociatedTokenAddressSync(mint.publicKey, seller.publicKey, false, tokenProgram)
    ...
    const auction = PublicKey.findProgramAddressSync(
      [Buffer.from("auction"), seller.publicKey.toBuffer(), mint.publicKey.toBuffer(), auctionId.toArrayLike(Buffer, "le", 8)],   // unique seed],
      program.programId
    )[0];
  ...
  //   });
    ...
    it("End to End Test Complete (two bidders)", async () => {
      const sellerSol_before = await provider.connection.getBalance(seller.publicKey);
      console.log("Seller SOL balance:", sellerSol_before.toString());
      const bidder1Sol_before = await provider.connection.getBalance(bidder1.publicKey);
      console.log("bidder1 SOL balance:", bidder1Sol_before.toString());
      const bidder2Sol_before = await provider.connection.getBalance(bidder2.publicKey);
  ...
  console.log("Auction created:", state);
      ...
      const winnerAta = getAssociatedTokenAddressSync(mint.publicKey, winner2, false, tokenProgram);
      const ataInfo = await provider.connection.getAccountInfo(winnerAta);
      if (!ataInfo) {
        const tx = new Transaction().add(
          createAssociatedTokenAccountIdempotentInstruction(
      ...
      const sellerSol_after = await provider.connection.getBalance(seller.publicKey);
      console.log("Seller SOL balance:", sellerSol_after.toString());
      const bidder1Sol_after = await provider.connection.getBalance(bidder1.publicKey);
      console.log("bidder1 SOL balance:", bidder1Sol_after.toString());
      const bidder2Sol_after = await provider.connection.getBalance(bidder2.publicKey);
